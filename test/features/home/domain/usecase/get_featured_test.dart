...
main() {
  ...
  setUp(() {
    mockHomeRepository = MockHomeRepository();
    getFeatured = GetFeatured(repository: mockHomeRepository);
  });
  ...
  test('should call getRecipes method', () {
    getFeatured(lat: lat, long: long);
    verify(mockHomeRepository.getFeatured(lat: lat, long: long));
  });
  test('should return List recipies when called', () async {
    when(mockHomeRepository.getFeatured(
            lat: anyNamed('lat'), long: anyNamed('long')))
        .thenAnswer((realInvocation) async => Right(restaurents));

    final result = await getFeatured(lat: lat, long: long);
    verify(mockHomeRepository.getFeatured(lat: lat, long: long));
