...
main() {
  ...
  GetTrendingGrocery getTrendingGrocery;

  setUp(() {
    mockHomeRepository = MockHomeRepository();
    getTrendingGrocery = GetTrendingGrocery(repository: mockHomeRepository);
  });
  ...
  test('should call getRecipes method', () {
    getTrendingGrocery(lat: lat, long: long);
    verify(mockHomeRepository.getTrendingGrocery(lat: lat, long: long));
  });
  test('should return List recipies when called', () async {
    when(mockHomeRepository.getTrendingGrocery(
            lat: anyNamed('lat'), long: anyNamed('long')))
        .thenAnswer((realInvocation) async => Right(restaurents));

    final result = await getTrendingGrocery(lat: lat, long: long);
    verify(mockHomeRepository.getTrendingGrocery(lat: lat, long: long));
